...
def main():
    global df, last_page
    #create df
    df = pd.DataFrame(columns=['supercategory_name','category_name','subcategory_name','product_name','product_brand','product_image','product_quantprice','price_old','price_new','price_eur'])
...
def post_processing():
    global df
    df = df.join(df.product_quantprice.str.split('-',1,expand=True).rename(columns={0:'raw_quantity', 1:'price/quantity'}))
    for i in df.index:
    ...
    df.product_brand.fillna('PAM PANORAMA', inplace =True)
    df.product_brand.replace('','PAM PANORAMA', inplace =True)
    df.product_brand.replace('.','PAM PANORAMA', inplace =True)
    df.loc[:,'product_name'] = df.product_name.str.capitalize()
    df.loc[:,'product_brand'] = df.product_brand.str.upper()
    df.loc[:,['product_name','product_brand','raw_quantity']] = df[['product_name','product_brand','raw_quantity']].apply(lambda x: x.str.strip())
    df.loc[:,'identifier'] = [f"{str((df.loc[index_n,'product_name'])).lower()} - {str((df.loc[index_n,'product_brand'])).lower()} - {df.loc[index_n,'raw_quantity']}" for index_n in df.index]
    date = datetime.datetime.now().strftime("%d_%m_%Y") 
    df.to_excel(f'pam_{selection}_{date}_.xlsx', index=False)
